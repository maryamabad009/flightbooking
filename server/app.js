...
mongoose.connect("mongodb+srv://numlf2012790:i5rlmsuJ54K5nkiX@cluster0.03anoyn.mongodb.net/test?retryWrites=true&w=majority", {
...
const start = async function() {
    const newPassword = await bcrypt.hash("1234", 2);
    Admin.create({
        adminId: "admin",
...
app.post("/addflight", async (req, res) => {
  ...
  ScheduleFlight.create({
    ...
    seats: seats,
    totalTime: totalTime,
    economyPrice: totalTime * 750,
    businessPrice: totalTime * 1750,
    firstClassPrice: totalTime * 3250,
    noOfPassengers: 0,
  });
});
...
app.post("/updateflight", async (req, res) => {
  ...
  ScheduleFlight.updateOne(
    { _id: req.body.id },
    {
      ...
      economyPrice: req.body.totalTime * 750,
      businessPrice: req.body.totalTime * 1750,
      firstClassPrice: req.body.totalTime * 3250,
    },
    function (err) {
      if (err) return handleError(err);
      // deleted at most one tank document
